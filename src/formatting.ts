 ...
 * Splits on newlines when possible, never inside HTML tags.
 * Each chunk is self-contained (no broken tags).
 */
export function splitMessage(text: string, maxLen = 4000): string[] {
  ...
  while (remaining.length > 0) {
    ...
    // Try space
    if (splitAt === -1) {
      const space = remaining.lastIndexOf(" ", searchEnd);
      if (space > maxLen * 0.3) {
        splitAt = space + 1;
      }
    }
    ...
    // Fix broken HTML tags — close any unclosed tags
    chunk = fixBrokenHtml(chunk);
 ...
 * Handles: <b>, <i>, <code>, <pre>, <blockquote>, <a>
 */
function fixBrokenHtml(html: string): string {
  const tags = ["pre", "code", "b", "i", "blockquote", "a"];
  ...
  for (const tag of tags) {
    ...
    const closes = (html.match(closePattern) || []).length;
